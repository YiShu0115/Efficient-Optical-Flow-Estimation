...
class CNNEncoder(torch.nn.Module):
    ...
    # @torch.autocast(device_type='cuda', enabled=False)  # 强制FP32
    def forward(self, img):
        img = F.avg_pool2d(img, kernel_size=2, stride=2)#1/2 scale
        x_8 = self.block_8_1(img)#1/8 scale

        img = F.avg_pool2d(img, kernel_size=2, stride=2)#1/4 scale
        x_8_2 = self.block_8_2(img)#1/8 scale

        x_8 = self.block_cat_8(torch.cat([x_8, x_8_2], dim=1))#1/8 scale

        img = F.avg_pool2d(img, kernel_size=2, stride=2)#1/8 scale
        x_16 = self.block_16_1(img)#1/16 scale

        x_16_2 = self.block_8_16(x_8)#1/16 scale

        x_16 = self.block_cat_16(torch.cat([x_16, x_16_2], dim=1))#1/16 scale
        # print("x_16 shape:", x_16.shape)        # 调试输出
        # print("pos_s16 shape:", self.pos_s16.shape)  # 调试输出
        # device = torch.device('cuda')
        # self.pos_s16 = torch.rand(2, 2, 24, 78) .to(device).half()
        x_16 = torch.cat([x_16, self.pos_s16], dim=1)#1/16 scale
