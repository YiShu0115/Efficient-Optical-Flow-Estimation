...
class TransformerLayer(torch.nn.Module):
    ...
    def forward(self, source, target):
        ...
        if self.ffn:
            assert source.shape[-1] is not None and message.shape[-1] is not None, "动态维度无法剪枝"
            combined = torch.cat([source, message], dim=-1)
            message = self.mlp(combined)
...
class FeatureAttention(torch.nn.Module):
    ...
    def forward(self, concat_features0):
        ...
        # reshape back
        concat_features0 = concat_features0.view(b, h, w, c).permute(0, 3, 1, 2).contiguous().half()  # [B, C, H, W]
...
class FlowAttention(torch.nn.Module):#没有用到
    """
