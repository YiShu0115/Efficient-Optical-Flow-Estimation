...
class Refine(torch.nn.Module):
    def __init__(self, feature_dim,context_dim, iter_context_dim, num_layers,inter_dim):
        super(Refine, self).__init__()

        self.conv1 = ConvBlock(2*feature_dim+context_dim+iter_context_dim+2, context_dim+iter_context_dim, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, iter_flow):#corrs, context, iter_context, flow0
        # device = torch.device('cuda')
        # self.radius_emb = torch.rand(1, 64, 24, 78) .to(device).half()
        #x = torch.cat([iter_flow, self.radius_emb], dim=1)
