...
class ConvBlock(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.hswish(self.norm1(self.conv1(x)))
        #x = self.relu(self.norm2(self.conv2(x)))
        x = self.hswish(self.norm2(self.conv2(x)))
    ...
    def forward_fuse(self, x):
        ...
        #x = self.relu(self.conv1(x))
        x = self.hswish(self.conv1(x))
        #x = self.relu(self.conv2(x))
        x = self.hswish(self.conv2(x))
        return x
...
##########1#########
class InitialPredict_CNNEncoder(nn.Module):
    def __init__(self, feature_dim_s16, context_dim_s16, feature_dim_s8, context_dim_s8):
        super(InitialPredict_CNNEncoder, self).__init__()
        ...
        self.cnet_s16 = nn.Sequential(
            ConvBlock(feature_dim_s8 + context_dim_s8, 64, kernel_size=3, stride=1, padding=1),  # Input: feature_dim_s16
            ConvBlock(64, 128, kernel_size=3, stride=1, padding=1),
            ConvBlock(128, 2, kernel_size=3, stride=1, padding=1)  # Output: context_dim_s16 + 2
        )
    def init_pos(self, batch_size, height, width, device, amp):
        ys, xs = torch.meshgrid(torch.arange(height, dtype=torch.half if amp else torch.float, device=device), torch.arange(width, dtype=torch.half if amp else torch.float, device=device), indexing='ij')
        ys = (ys-height/2)
    ...
    def forward(self, img):
        ...
        flow0 = self.cnet_s16(x_16)

        return x_16, flow0, x_8
######################
