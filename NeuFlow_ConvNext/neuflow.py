...
from NeuFlow_ConvNext import backbone_v7
from NeuFlow_ConvNext import transformer
from NeuFlow_ConvNext import matching
from NeuFlow_ConvNext import corr
from NeuFlow_ConvNext import refine
from NeuFlow_ConvNext import upsample
from NeuFlow_ConvNext import config
...
class NeuFlow(torch.nn.Module,
    ...
    def __init__(self):
        ...
        self.info_head_s16 = torch.nn.Sequential(
            torch.nn.Conv2d(config.feature_dim_s16, config.feature_dim_s16, 3, padding=1),
            torch.nn.GELU(),
            torch.nn.Conv2d(config.feature_dim_s16, 4, 3, padding=1)
        )
        
        self.info_head_s8 = torch.nn.Sequential(
            torch.nn.Conv2d(config.feature_dim_s8, config.feature_dim_s8, 3, padding=1),
            torch.nn.GELU(),
            torch.nn.Conv2d(config.feature_dim_s8, 4, 3, padding=1)
        )
        
        # Each head predicts 8 homography parameters per local region
        ...
        self.refine_s16 = refine.Refine(config.context_dim_s16, config.iter_context_dim_s16, num_layers=2, levels=1, radius=4, inter_dim=128)
        self.refine_s8 = refine.Refine(config.context_dim_s8, config.iter_context_dim_s8, num_layers=2, levels=1, radius=4, inter_dim=96)
    ...
    def forward(self, img0, img1, iters_s16=1, iters_s8=8):
        ...
        flow_list = []
        info_list = []
        # Add homography predictions list for geometric constraints
        ...
        for i in range(iters_s16):
            ...
            if self.training:
                ...
                info_s16 = self.info_head_s16(feature0_s16)
                info_s16 = F.interpolate(info_s16, scale_factor=16, mode='bilinear')
                flow_list.append(up_flow0)
                info_list.append(info_s16)
        ...
        for i in range(iters_s8):
            ...
            if self.training or i == iters_s8 - 1:
                ...
                info_s8 = self.info_head_s8(feature0_s8)
                info_s8 = F.interpolate(info_s8, scale_factor=8, mode='bilinear')
                flow_list.append(up_flow0)
                info_list.append(info_s8)
